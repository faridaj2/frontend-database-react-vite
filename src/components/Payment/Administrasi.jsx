...
export default function Administrasi() {
	...
	const getStudents = (search = '') => {
		http.get(`/api/pspdb/search/listed?search=${search}&formal=${sekolah}&kelamin=${kelamin}`)
	...
	const getPayment = () => {
		http.get('/api/pspdb/payment/read')
	...
	return (
		<>
			...
			{filter && (
				<>
					<div className="border p-2 rounded-xl shadow mb-2 flex gap-2 bg-white">
						...
						<Select variant="bordered" color="primary" label="Pelunasan" size="sm"
							selectedKeys={[lunas]}
						...
						>
							...
							<SelectItem key="belum">
								Sudah Lunas
							</SelectItem>
							<SelectItem key="sudah">
								Belum Lunas
							</SelectItem>
						</Select>
					</div>
			...
			<div className={`relative overflow-x-auto bg-white rounded-xl shadow ${loading && 'hidden'}`}>
				<table className="w-full text-sm text-left overflow-auto">
					<thead className="text-xs text-gray-500 font-sans">
						<tr className="h-10">
							<th className="px-6">Nama Siswa</th>
							{columns?.map((item, index) => (
								<th className="px-6 py-3 text-nowrap" key={`${index}th`}>
									{item.nama_kolom}
								</th>
							))}
						</tr>
					...
					<tbody>
						<tr className={`${!v.includes('sekolah') && 'hidden'} bg-gray-100`}>
							<td className="px-6 border text-gray-600"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-school-icon lucide-school"><path d="M14 22v-4a2 2 0 1 0-4 0v4" /><path d="m18 10 3.447 1.724a1 1 0 0 1 .553.894V20a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2v-7.382a1 1 0 0 1 .553-.894L6 10" /><path d="M18 5v17" /><path d="m4 6 7.106-3.553a2 2 0 0 1 1.788 0L20 6" /><path d="M6 5v17" /><circle cx="12" cy="9" r="2" /></svg></td>
							{columns?.map((item, index) => <td key={`${index}tr2`} className="px-6 text-xs text-cente py-2 border"><div className="uppercase bg-blue-500 p-1 text-white inline-block rounded-full px-4 font-bold tracking-wide text-[0.6rem]">{item.formal == '' ? 'Semua' : item.formal}</div></td>)}
						</tr>
						...
						{students &&
							students?.map((item, index) => (
								<tr
									key={`table-${index}`}
									className="bg-white border-b dark:bg-gray-800 border-gray-200 text-gray-700"
								>
									<th
										scope="row"
										className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap font-semibold cursor-pointer"
									>
										{item.nama_siswa}
									</th>
			...
			<ModalSelectStudent modal={modalStudent} setModal={setModalStudent} searchBar={searchStudents} students={sstudents} />
		</>
	)
}
